...
1. Step 1: Generation
   ...
   - Saves the output to `output/step1_generated_qa_{generator_prompt}.json`.
...
- `step1_generation.py` — generates Q&A pairs and writes `output/step1_generated_qa_{generator_prompt}.json`
...
- `output/step1_generated_qa_{generator_prompt}.json` — generated Q&A dataset with metadata per item
...
- Step 5 reads `output/step1_generated_qa_{generator_prompt}.json`, `output/step3_human_labels.json`, and `output/step4_llm_judge_labels.json`. Missing label files are treated as empty, so pass/agreement rates will be zero until those steps have been run.
